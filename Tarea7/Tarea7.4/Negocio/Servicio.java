...
public class Servicio
{
  ...
  public Response alta(@FormParam("articulo")  Articulo articulo) throws Exception
  {
    ...
    return Response.status(400).entity(j.toJson(new Error("Ingrese el nombre"))).header("Access-Control-Allow-Origin", "*").build();

    if (articulo.descripcion == null || articulo.descripcion.equals(""))
      return Response.status(400).entity(j.toJson(new Error("Ingrese la descripcion"))).header("Access-Control-Allow-Origin", "*").build();

    if (articulo.precio == null || articulo.precio<1)
      return Response.status(400).entity(j.toJson(new Error("Ingrese el precio"))).header("Access-Control-Allow-Origin", "*").build();

    if (articulo.cantidad_almacen == null || articulo.cantidad_almacen<1)
      return Response.status(400).entity(j.toJson(new Error("Ingrese la cantidad disponible en almacen"))).header("Access-Control-Allow-Origin", "*").build();

    if (articulo.foto == null || articulo.foto.equals(""))
      return Response.status(400).entity(j.toJson(new Error("Por favor, suba una foto"))).header("Access-Control-Allow-Origin", "*").build();
    ...
    try
    {
      ...
      try
      {
        ...
        {
          if (rs.next())
             return Response.status(400).entity(j.toJson(new Error("Ya existe un articulo igual"))).header("Access-Control-Allow-Origin", "*").build();
        }
        finally
        {
          rs.close();
    ...
    try
    {
      ...
      try
      {
        ...
        try
        {
          ...
          return Response.status(400).entity(j.toJson(new Error("No existe el articulo"))).header("Access-Control-Allow-Origin", "*").build();
        }
        finally
        {
          rs.close();
  ...
  @POST
  @Path("consultar_carrito")
  @Consumes(MediaType.APPLICATION_FORM_URLENCODED)
  @Produces(MediaType.APPLICATION_JSON)
  public Response consultar_carrito() throws Exception
  {
    Connection conexion= pool.getConnection();
  ...
  @Consumes(MediaType.APPLICATION_FORM_URLENCODED)
  @Produces(MediaType.APPLICATION_JSON)
  public Response borrar_articulo_carrito(@FormParam("id_articulo") Integer id_articulo) throws Exception
  {
    Connection conexion= pool.getConnection();
    try
    {
  ...
  @Consumes(MediaType.APPLICATION_FORM_URLENCODED)
  @Produces(MediaType.APPLICATION_JSON)
  public Response borrar_carrito() throws Exception
  {
    Connection conexion= pool.getConnection();
    try
    {
